...
void lbm_comm_init( lbm_comm_t * mesh_comm, int rank, int comm_size, int width, int height )
{
	...
	if (height % nb_y != 0 || width % nb_x != 0)
        {	
          printf("Can't get a 2D cut for current problem size and number of processes.\n");
          exit(0);
        }
...
void lbm_comm_sync_ghosts_horizontal( lbm_comm_t * mesh, Mesh *mesh_to_process, lbm_comm_type_t comm_type, int target_rank, int x )
{
	...
	{
		case COMM_SEND:
				MPI_Send( Mesh_get_line( mesh_to_process, x ), mesh->height - 2, MPI_DOUBLE, target_rank, 0, MPI_COMM_WORLD);
			break;
		case COMM_RECV:
				MPI_Recv( Mesh_get_line( mesh_to_process, x ), mesh->height - 2, MPI_DOUBLE, target_rank, 0, MPI_COMM_WORLD,&status);
			break;
		default:
